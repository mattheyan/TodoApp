...
(function() {
	...
	Model.prototype = {
		...
		notifyBeforeContextReady: function () {
			if (this._ruleQueue) {
				...
				// register all rules pending registration
				this._ruleQueue.forEach(function (rule) {
					rule.register();
				});
				...
				// execute all init rules of instance loaded before rule registration
				this._ruleQueue.forEach(function (rule) {
	...
	Property.mixin({
		...
		_getter: function Property$_getter(obj, skipTypeCheck) {
			...
			// initialize default values for loaded instances
			if (!obj.hasOwnProperty(this._fieldName) && LazyLoader.isLoaded(obj)) {
		...
		conditionIf: function (options, error) {
	...
	ObjectMeta.mixin({
		...
		pendingInit: function ObjectMeta$pendingInit(prop, value) {
			...
			if (arguments.length > 1) {
				if (value) {
					...
					Observer.raisePropertyChanged(this._obj, prop.get_name());
				}
				else {
		...
		// stores the condition for the current instance
	...
	function Rule(rootType, options) {
		...
		rootType.get_model().registerRule(this);
	}
	...
	Rule.mixin({
		...
		register: function Rule$register() {
			...
			var execute = function (rule, obj, args) {
				...
				try {
					rule.rootType.get_model().beginValidation();
					rule.execute.call(rule, obj, args);
				}
				finally {
					rule.rootType.get_model().endValidation();
				}
			};
					...
					this.predicates.forEach(function (predicate) {
						predicate.addChanged(
							function (sender, args) {
								...
								// Otherwise, just mark the property as pending initialization
								else {
	...
	function ConditionRule(rootType, options) {
		...
		Object.defineProperty(this, "conditionType", { 
			value: options.conditionType || Rule.ensureConditionType(options.name, rootType, options.category || ConditionType.Error)
		});
		...
		if (options.message) {
			Object.defineProperty(this, "message", { value: options.message });
		}
		if (options.properties) {
			Object.defineProperty(this, "properties", { value: options.properties });
		}
	...
	ConditionRule.mixin({
		...
		execute: function ConditionRule$execute(obj) {
			...
			if (assert !== undefined) {
				this.conditionType.when(
					this.message instanceof Function ? this.message() : this.message,
					obj, assert, 
					this.properties instanceof Function ? this.properties() : this.properties);
			}
		},
	...
	function CalculatedPropertyRule(rootType, options) {
		...
		// store the property being calculated
		this._property = options.property instanceof Property ? options.property : rootType.property(options.property);

		// store the calculation function
		this._calculate = options.calculate || options.fn;

		// indicate that the rule is responsible for returning the value of the calculated property
		options.returns = [this._property];
	...
	CalculatedPropertyRule.mixin({
		execute: function CalculatedPropertyRule$execute(obj) {
			...
			// Convert string functions into compiled functions on first execution
			if (this._calculate.constructor === String) {
				this._calculate = this.rootType.compileExpression(this._calculate);
			}
			...
			if (prop.get_isList()) {
				...
				var newList = this._calculate.apply(obj);
				...
				prop.value(obj, this._calculate.apply(obj), { calculated: true });
			}
		},
		toString: function () {
			return "calculation of " + this._property._name;
		}
	});
	...
	function RequiredIfRule(rootType, options) {
		...
		///		The options for the rule, including:
		///			property:			the property being validated (either a Property instance or string property name)
		///			isRequired:			a predication function indicating whether the property should be required
		///			compareSource:		the source property to compare to (either a Property or PropertyChain instance or a string property path)
		///			compareOperator:	the relational comparison operator to use (one of "Equal", "NotEqual", "GreaterThan", "GreaterThanEqual", "LessThan" or "LessThanEqual")
		...
		if (options.isRequired instanceof Function) {
	...
	StringLengthRule.mixin({
		...
		isValid: function StringLengthRule$isValid(obj, prop, val) {
			return (!this.min || val.length >= this.min) && (!this.max || val.length <= this.max);
		},
	...
	ConditionType.prototype = {
		...
		when: function ConditionType$when(message, target, condition, properties) {
	...
	// #region ExoWeb.Mapper.TriggersRoundtripRule
	//////////////////////////////////////////////////
	...
	function TriggerRoundtripRule(property) {
		...
		// Call the base rule constructor
		Rule.apply(this, [property.get_containingType(), { onChangeOf: [property] }]);
	}

	// setup the inheritance chain
	TriggerRoundtripRule.prototype = new Rule();
	TriggerRoundtripRule.prototype.constructor = TriggerRoundtripRule;

	// implement the execute method
	TriggerRoundtripRule.mixin({
	...
	Rule.triggerRoundtrip = TriggerRoundtripRule;
	...
	function conditionFromJson(model, code, json) {
		...
		json.forEach(function (conditionJson) {
			var condition = new Condition(type, conditionJson.message, null, null, "server");

			// process each condition target
			conditionJson.targets.forEach(function (target) {
				tryGetJsType(serverSync._model, target.instance.type, null, false, function (jstype) {
					tryGetEntity(serverSync._model, serverSync._translator, jstype, target.instance.id, null, LazyLoadEnum.None, function (instance) {
	...
	function objectFromJson(model, typeName, id, json, callback, thisPtr) {
		...
		//					ExoWeb.trace.log("propInit", "{0}({1}).{2} = {3}", [typeName, id, propName, propData]);
			...
			if (obj) {
				...
				// ensure the init existing event is raised once loading completes
				if (!instancesPendingInit) {
					instancesPendingInit = [obj];

					// Only raise init existing event when the current loading batch completes
					Batch.whenDone(function () {
						var instancesToInit = instancesPendingInit;
						instancesPendingInit = null;

						// Raise init events if registered.
						instancesToInit.forEach(function (obj) {
	...
	Adapter.mixin({
		...
		_onTargetChanged: function Adapter$_onTargetChanged(sender, args) {
			...
			if (numProps > 1 && args.triggeredBy !== this._propertyChain.lastProperty()) {
				...
				if (this._conditionsChangedHandler) {
					this.addConditionsChanged(this._conditionsChangedHandler, this._propertyChain);
				}
			}
		...
		get_conditions: function Adapter$get_conditions() {
			if (!this._conditions) {
				...
				if (target) {
					var conditions = this._conditions = target.meta.conditions(this._propertyChain.lastProperty());
					Observer.makeObservable(this._conditions);
					var handler = this._conditionsChangedHandler = function (sender, args) {
						if (args.add) {
